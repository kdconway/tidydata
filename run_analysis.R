run_analysis <- function () {
    ...
    col.keep <- grep ("mean|std", features)
    ...
    #recode activity values as factors. Set levels as previously extracted vector "activities"    
    data.all [, activity := factor (activity)]
    levels (data.all$activity) <- activities

    
    #perform mean of all columns     
    data.means <- data.all [, lapply (.SD, mean), by=.(id, activity), .SDcols=4:ncol (data.all)]
